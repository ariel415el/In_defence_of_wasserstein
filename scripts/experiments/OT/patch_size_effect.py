...
def main():
    ...
    with torch.no_grad():
        ...
        patch_sizes = [1, 3,7,11,22, im_size]
        ...
        for p in patch_sizes:
            for name, batch in named_batches:
                print(name, p)
                dists[name].append(MiniBatchPatchLoss(dist, p=p, s=stride)(batch, data))
...
def plot(patch_sizes, dists, named_batches):
    ...
    plt.label(f"Comparing {b} and {n_data} images with stride {stride}")
    plt.legend()
