...
class Generator(nn.Module):
    def __init__(self, z_dim, output_dim, nfilter=64, nfilter_max=512, bn=True, res_ratio=0.1, **kwargs):
        super().__init__()
        s0 = self.s0 = 4
        nf = self.nf = nfilter
        ...
        # Submodules
        nlayers = int(np.log2(output_dim / s0))
...
class Discriminator(nn.Module):
    def __init__(self, input_dim, nfilter=64, nfilter_max=512, res_ratio=0.1, **kwargs):
        super().__init__()
        s0 = self.s0 = 4
        nf = self.nf = nfilter
        nf_max = self.nf_max = nfilter_max
        ...
        # Submodules
        nlayers = int(np.log2(input_dim / s0))
