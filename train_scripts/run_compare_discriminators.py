...
if __name__ == '__main__':
    ...
    for gen_arch in ['FC', 'DCGAN']:
        project_name = f"paper_figures_9-6_G-{gen_arch}"
        commands = dict()
        for data_name, (data_path, args, N) in data_map.items():
            for z_prior in [f'const={M}']:
                base_command = f"python3 train.py --data_path {data_path} {args}" \
                                        ...
                                        f" --gen_arch {gen_arch} "# --wandb"
                for reg_name, reg in [('GP', '--gp_weight 10')]:#, ('WC', '--weight_clipping 0.01'), ('SN', '--spectral_normalization')]:
                    for disc_arch in ['FC-nf=1024', 'CNN-GAP=True']:#, 'CNN-GAP=False']:
                        train_name = f"{data_name}-{z_prior}_WGAN-{reg_name}-{disc_arch}"
                        train_command = base_command + f" --disc_arch {disc_arch} --lrD 0.001 --lrG 0.0001" \
