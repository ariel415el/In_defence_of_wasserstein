...
if __name__ == '__main__':
    ...
    data_map = {
        "ffhq": (f'{data_root}/FFHQ/FFHQ', ' --center_crop 90 --limit_data 10000', 1000),
    ...
    for gen_arch in ['FC']:#, 'DCGAN']:
        project_name = f"paper_figures_1-10_G-{gen_arch}_M=N_1K"
        commands = dict()
        for data_name, (data_path, args, N) in data_map.items():
            for z_prior in [f'const={M}']:
                ...
                for disc_arch in ['FC-nf=1024', 'CNN-GAP=True',  'CNN-GAP=False']:#, 'SCNN-p=16-s=8']:
