...
def create_plot(dataset_name):
    width = len(raw_train_names[0]) + len(metrics[0])
    height = len(raw_train_names)
    s = 5
    n = 3
    COLORS = ['r', 'g', 'b']
    fig, axes = plt.subplots(height, width, figsize=(width * s, height * s))
...
def create_table(dataset_name):
    ...
    for r in range(height):
        for c in range(width):
            metric_name, metric_file_name, line_type = metrics[c][0]
            for i in range(2):
